...
def jeopardy(page: int = 1):
    '''
    Crawl through http://www.j-archive.com/, specifically through a page representing
    a Jeopardy game with id page (the parameter)
    '''
    ...
    # Category names are separate from the following code because
    # we want to have them all before we start matching
    ...
    for text in soup.findAll({'td'}):
        ...
        if identifier is not None and identifier in questions_answers:
            ...
            if text_str is not None:
                ...
                category: str = ""
                # Just set Final Jeopardy clues to be worth $1000
                amount: int = 1000
                ...
                # Write to a text file with your own delimiter, since commas may appear in 
                # clues/answers so we won't be able to split the content properly later.
                # The following delimiter is likely not going to appear in content from the site :)
                delimiter: str = "{+~}"
                # We've already previously made sure that all other values being output here are not None,
                # so this is the last check.
                if (category is not None):
                    ...
                    # Just don't bother writing data that will produce a UnicodeError
                    # (instead of specifying the encoding to avoid this error)
...
# 6665 pages total on the site, wow! Just doing enough for good variety.
